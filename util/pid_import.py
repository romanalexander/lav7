# Imports PHP constant declaration statement to Go constant declaration statement

p = '''const LOGIN_PACKET = 0x8f;
...
def parse_consts():
    ...
    for line in p.split("\n"):
        ...
        if l == "":
            continue
        ll = l.split("=")
        parsedTabs.append((''.join(map(lambda x: x[0].upper() + x[1:].lower(), ll[0].split("_")))[:-6] + "Head", int(ll[1], 0)))

    return parsedTabs

def format_consts(parsedTabs: list):
    out = ""
    out += "const (\n"
    diff, last = parsedTabs[0][1], int('0x8f', 0)
    for k, elem in enumerate(parsedTabs):
        if diff == elem[1] - k:
            last = elem[1]
            if k == 0:
                out += "    %s byte = %s + iota\n" % (elem[0], hex(elem[1]))
                ...
                out += "    %s\n" % elem[0]
        elif (elem[1] - k) - diff > 0:
            for i in range(last + 1, elem[1]):
                out += "    _ // %s is skipped\n" % hex(i)
            out += "    %s\n" % elem[0]
            last = elem[1]
            diff = elem[1] - k
        else:
            raise LookupError("Invalid diff: %s" % (elem[1] - k))

    out += ")\n"
    return out

if __name__ == "__main__":
    print(format_consts(parse_consts()), end="")
